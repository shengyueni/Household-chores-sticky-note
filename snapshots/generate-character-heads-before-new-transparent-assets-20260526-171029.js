...
function cropUpperBody(width, height, rgba) {
  ...
  const cropHeight = Math.max(1, Math.round(bounds.height * 0.58));
  const cropWidth = Math.max(bounds.width * 1.08, cropHeight * 0.9);
...
function renderHead(width, height, rgba, crop, size = 128) {
  ...
  const scale = Math.min((size * 0.9) / crop.width, (size * 0.9) / crop.height);
...
for (const id of ids) {
  ...
  const input = fs.existsSync(transparent) ? transparent : body;
  const png = parsePng(input);
  const rgba = toRgba(png);
  clearEdgeBackground(png.width, png.height, rgba);
